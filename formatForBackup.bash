...
declare		ALGORITHM_CHECKSUM='blake2'
declare		SUBVOLUME_BACKUP='Documents'
declare		SUBVOLUME_SNAPSHOTS='.snapshots'
declare		NAME_USER="${LOGNAME:-$(whoami)}"
declare		NAME_GROUP='users'
...
# Whilst these directories/subvolumes were created by root, the owner can be less privileged.
	if (( RETURN_CODE == 0 )) && ! sudo chown $VERBOSE -- "$OWNER" $SUBVOLUMES './'; then
		echo "'chown $OWNER' failed" >&2
	...
	case "$OPTION" in
		...
		?)
			...
			printf '\t%-32s # %s.\n'\
				...
				"Default '$NAME_DECRYPTED_STORAGE_VOLUME'"\
				'[-f <filesystem-label>]'\
				"Default '$LABEL_FILESYSTEM'"\
				'[-c <checksum-algorithm>]'\
				"Default '$ALGORITHM_CHECKSUM'"\
				'[-b <location of backup>]'\
				"Where the backup is stored. Default '$SUBVOLUME_BACKUP'"\
				'[-s <location of snapshots>]'\
				"Where snapshots are stored. Default '$SUBVOLUME_SNAPSHOTS'"\
				'[-u <user-name>]'\
				"Define file-owner. Default '$NAME_USER'"\
				'[-g <group-name>]'\
				"Define file-group. Default '$NAME_GROUP'"\
