...
CONFIRM_FILESYSTEM_TYPE	:= [[ "$$(stat --file-system --format='%T' ./)" == 'btrfs' ]]
SUBVOLUME_LIST_TO_PATHS	:= sed -e '1,2d' -e 's/^.*\t//'
...
# Delete any snapshots beyond the maximum number, oldest first.
cullSnapshots:
	$(CONFIRM_FILESYSTEM_TYPE);
